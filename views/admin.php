 ...
 * POST処理
***/
if (isset($_POST['mail'], $_POST['subject'], $_POST['reply'], $_POST['letters']) && wp_verify_nonce($_POST['_wpnonce'], 'ccc')) {
    //メール
    if (is_email($_POST['mail']) == true) {
        update_option('ccc_plugin_value_mail', esc_html($_POST['mail']));
    } else {
    ...
    if (is_email($_POST['reply']) == true) {
        update_option('ccc_plugin_value_reply', esc_html($_POST['reply']));
    } else {
    ...
    //ログイン時のチェック
    $login_flg = esc_html($_POST['login_flg']);
    update_option('ccc_plugin_value_login_flg', $login_flg);
    //ログイン時のチェック
    $ga_push = esc_html($_POST['ga_push']);
...
<tbody>
	<tr>
		...
		<td>
			<input type="text" name="mail" value="<?php echo esc_html($mail); ?>" class="p80" />
	...
	<tr>
		...
		<td>
			<input type="text" name="subject" value="<?php echo esc_html($subject); ?>" class="p80" />
		</td>
	</tr>
	<tr>
		<th>
		...
		<td>
			<input type="text" name="reply" value="<?php echo esc_html($reply);?>" class="p80" />
	...
	<tr>
		...
		<td>
			<input type="text" name="letters" value="<?php echo esc_html($letters); ?>" class="p20" /> <?php _e('characters or more', $this->textdomain);?>
